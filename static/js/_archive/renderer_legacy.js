...
function drawMap(playerPos, visibleMap, enemies, corpses, npcs) {
    ...
    // ctx.fillStyle = "#001e1e"; // FORCE TEAL/DARK OCEAN to prove script runs. If this shows, script is running.
    ...
    for (const tileKey in visibleMap) {
        // Safe Parse and Force Number Type
        ...
        // DEBUG: Check first tile with Strict Types
        if (Object.keys(visibleMap)[0] === tileKey) {
            console.log(`DEBUG DRAW: TileZ=${worldZ} (${typeof worldZ}) vs PlayerZ=${playerZ} (${typeof playerZ})`);
        }

        // Strict Equality now safe
