        ...
        enemies.forEach(enemy => {
            ...
            if (Math.abs(ex - cameraPos[0]) <= DRAW_RADIUS_X && Math.abs(ey - cameraPos[1]) <= DRAW_RADIUS_Y) {
                ...
                if (name.includes("knife")) {
                    console.log("RENDERER: Found Knife Goblin!");
                    // alert("RENDERER: Found Knife Goblin!"); // Uncomment if desperate
                ...
                if (eImg.complete) {
                    ctx.drawImage(eImg, drawX, drawY, TILE_SIZE, TILE_SIZE);
                } else {
                    ctx.fillStyle = 'red';
                    ctx.fillRect(drawX, drawY, TILE_SIZE, TILE_SIZE);
                }
                ...
                // SHOW RAW NAME - FORCE VISIBILITY
                ctx.fillStyle = 'yellow';
                ctx.fillText(`RAW:${name}`, drawX, drawY - 30);
